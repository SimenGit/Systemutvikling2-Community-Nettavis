...
router.post('/', (req, res) => {
  ratingDao.checkUser(req.body, (status, data) => {
    console.log(data);
    if (data.length < 1) {
      console.log('finnes ikke fra før');
      ratingDao.createOne(req.body, (status, data) => {
        res.status(status);
        res.json(data);
      });
    } else {
      console.log('finnes fra før');
      ratingDao.updateOne(req.body, (status, data) => {
        res.status(status);
        res.json(data);
      });
    }
...
router.delete('/:article_fk', (req, res) => {
  ratingDao.deleteOne(req.params.article_fk, (status, data) => {
    res.status(status).json(data);
  });
});
